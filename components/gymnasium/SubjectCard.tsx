...
export default function SubjectCard({ subject, topics, gamePoints, userId }: SubjectCardProps) {
  ...
  return (
    <motion.div
      ...
      className="bg-white dark:bg-gray-900 shadow-sm border border-gray-100 dark:border-gray-800 rounded-xl overflow-hidden"
      whileHover={{
    ...
    >
      ...
      >
        <div className="flex justify-between items-center">
          <div>
            <h2 className="font-bold text-gray-800 dark:text-gray-100 text-xl">{subject.title}</h2>
            <p className="mt-1 text-gray-500 dark:text-gray-400 text-sm">
              {topics.length} {topics.length === 1 ? 'topic' : 'topics'} available
            </p>
          </div>
          <div className="flex items-center space-x-3">
            <div className="flex items-center bg-amber-50 dark:bg-amber-900/20 px-3 py-1.5 border dark:border-amber-800/30 rounded-full font-medium text-amber-700 dark:text-amber-400 text-sm">
      ...
      <AnimatePresence>
        {expanded && (
          <motion.div
            ...
            className="border-gray-100 dark:border-gray-700 border-t overflow-hidden"
          >
            <div className="space-y-4 bg-gray-50 dark:bg-gray-800/50 p-5">
              {topics.length > 0 ?
                ...
                <p className="py-4 text-gray-500 dark:text-gray-400 text-center">No topics available</p>
              }
            </div>
          </motion.div>
        )}
